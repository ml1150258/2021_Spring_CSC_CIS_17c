...
void simPltX(int,int,int[],int);        //Simple X plot
void smPltXY(int,int,int[],int);        //Simple XY plot
...
int main(int argc, char** argv) {
    ...
    prntAll(sequence,up,down,L);
    cout<<"Theory says according to the Pidgeon Hole Principle"<<endl;
    cout<<"Sequence Length <= Max of Increasing * Max of Decreasing Sequence Length"<<endl;
    int maxsequ=maxSeq(up,L);
    int maxseqd=maxSeq(down,L);
    int maxutq=maxsequ*maxseqd;
    cout<<"    "<<L<<" <= "<<maxsequ<<" * "<<maxseqd<<" = "<<maxutq<<endl;
    
    //Plot the data
    simPltX(100,100,sequence,L);
...
void prntAll(int s[],int u[],int d[],int n){
    cout<<"    Sequence  Increasing Decreasing"<<endl;
    for(int i=0;i<n;i++){
        cout<<setw(10)<<s[i]
            <<setw(10)<<u[i]
            <<setw(10)<<d[i]<<endl;
...
void simPltX(int xd,int yd,int data[],int n){
    char surf[yd][xd];
    ...
    for(int j=0;j<n;j++){
        int datay=data[j]*(yd-1)/(maxY-minY)-1;
